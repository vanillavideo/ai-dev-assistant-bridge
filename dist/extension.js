"use strict";var ue=Object.create;var B=Object.defineProperty;var ge=Object.getOwnPropertyDescriptor;var ve=Object.getOwnPropertyNames;var be=Object.getPrototypeOf,me=Object.prototype.hasOwnProperty;var we=(e,t)=>{for(var o in t)B(e,o,{get:t[o],enumerable:!0})},q=(e,t,o,l)=>{if(t&&typeof t=="object"||typeof t=="function")for(let n of ve(t))!me.call(e,n)&&n!==o&&B(e,n,{get:()=>t[n],enumerable:!(l=ge(t,n))||l.enumerable});return e};var M=(e,t,o)=>(o=e!=null?ue(be(e)):{},q(t||!e||!e.__esModule?B(o,"default",{value:e,enumerable:!0}):o,e)),fe=e=>q(B({},"__esModule",{value:!0}),e);var Be={};we(Be,{activate:()=>Ae,deactivate:()=>Pe});module.exports=fe(Be);var a=M(require("vscode")),J=M(require("http")),X=M(require("fs")),Z=M(require("path")),T,b,W,L,$,F,A,v=3737,x,R,N;function s(e,t,o){let n=`[${new Date().toISOString()}] [${e}]`,i=o?`${n} ${t} ${JSON.stringify(o)}`:`${n} ${t}`;b.appendLine(i),e==="ERROR"&&console.error(i)}function ke(e){return e instanceof Error}function y(e){return ke(e)?e.message:typeof e=="string"?e:JSON.stringify(e)}async function f(e){return e.workspaceState.get("tasks",[])}async function j(e,t){await e.workspaceState.update("tasks",t)}async function ee(e,t,o="",l="other"){let n=await f(e),i={id:Date.now().toString(),title:t,description:o,status:"pending",category:l,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};return n.push(i),await j(e,n),i}async function D(e,t,o){let l=await f(e),n=l.find(i=>i.id===t);n&&(n.status=o,n.updatedAt=new Date().toISOString(),await j(e,l))}async function te(e,t){let l=(await f(e)).filter(n=>n.id!==t);await j(e,l)}function m(){return a.workspace.getConfiguration("aiFeedbackBridge")}async function H(e,t){let o=m();await o.update(e,t,a.ConfigurationTarget.Workspace),s("DEBUG",`Config updated: ${e} = ${t}`,{scope:"Workspace",newValue:o.get(e)})}async function z(){if(N&&R){let e=await f(R);N.webview.html=await S(m(),v,e),s("DEBUG","Settings panel refreshed")}}var oe="aiFeedbackBridge.portRegistry",Y=3737,he=50;async function ae(e){return e.globalState.get(oe,[])}async function U(e,t){await e.globalState.update(oe,t)}async function ye(e){let t=await ae(e),o=a.workspace.name||"No Workspace",l=a.workspace.workspaceFolders?.[0]?.uri.fsPath||"no-workspace",n=Date.now()-60*60*1e3,i=t.filter(d=>d.timestamp>n),p=i.find(d=>d.workspace===l);if(p)return s("INFO",`Reusing existing port ${p.port} for workspace`),p.timestamp=Date.now(),await U(e,i),p.port;let r=new Set(i.map(d=>d.port)),c=Y;for(let d=0;d<he;d++){let u=Y+d;if(!r.has(u)&&await Ce(u)){c=u;break}}return i.push({port:c,workspace:l,timestamp:Date.now()}),await U(e,i),s("INFO",`Auto-assigned port ${c} for workspace: ${o}`),c}async function Ce(e){return new Promise(t=>{let o=J.createServer();o.once("error",l=>{l.code==="EADDRINUSE"?t(!1):t(!0)}),o.once("listening",()=>{o.close(),t(!0)}),o.listen(e)})}async function xe(e,t){let o=await ae(e),l=a.workspace.workspaceFolders?.[0]?.uri.fsPath||"no-workspace",n=o.filter(i=>!(i.port===t&&i.workspace===l));await U(e,n),s("INFO",`Released port ${t}`)}async function Ie(e){if(N){N.reveal(a.ViewColumn.One);let n=await f(e);N.webview.html=await S(m(),v,n);return}let t=a.window.createWebviewPanel("aiFeedbackBridgeSettings","AI Feedback Bridge Settings",a.ViewColumn.One,{enableScripts:!0,retainContextWhenHidden:!0});N=t,t.onDidDispose(()=>{N=void 0},null,e.subscriptions);let o=m(),l=await f(e);t.webview.html=await S(o,v,l),t.webview.onDidReceiveMessage(async n=>{switch(n.command){case"updateSetting":await H(n.key,n.value),s("INFO",`Setting updated: ${n.key} = ${n.value}`);break;case"reload":let i=await f(e);t.webview.html=await S(m(),v,i);break;case"runNow":try{let r=await G(e,!0);r?(await O(r,{source:"manual_trigger",timestamp:new Date().toISOString()}),s("INFO","[Run Now] Manually triggered all enabled reminders")):a.window.showInformationMessage("No enabled categories (check settings)")}catch(r){s("ERROR","[Run Now] Failed to send message",{error:y(r)}),a.window.showErrorMessage("Failed to send reminders")}break;case"injectScript":_();break;case"saveNewTask":try{let r=await ee(e,n.title,n.description,n.category),c=await f(e);t.webview.html=await S(m(),v,c)}catch(r){a.window.showErrorMessage(`Failed to create task: ${y(r)}`)}break;case"updateTaskField":try{let r=await f(e),c=r.find(d=>d.id===n.taskId);if(c){n.field==="title"?c.title=n.value:n.field==="description"&&(c.description=n.value),c.updatedAt=new Date().toISOString(),await j(e,r);let d=await f(e);t.webview.html=await S(m(),v,d)}}catch(r){a.window.showErrorMessage(`Failed to update task: ${y(r)}`)}break;case"updateTaskStatus":try{await D(e,n.taskId,n.status);let r=await f(e);t.webview.html=await S(m(),v,r)}catch(r){a.window.showErrorMessage(`Failed to update status: ${y(r)}`)}break;case"createTask":await a.commands.executeCommand("ai-feedback-bridge.addTask");let p=await f(e);t.webview.html=await S(m(),v,p);break;case"openTaskManager":await a.commands.executeCommand("ai-feedback-bridge.listTasks");break;case"clearCompleted":try{let c=(await f(e)).filter(u=>u.status==="completed");for(let u of c)await te(e,u.id);let d=await f(e);t.webview.html=await S(m(),v,d)}catch(r){a.window.showErrorMessage(`Failed to clear completed tasks: ${y(r)}`)}break}},void 0,e.subscriptions)}async function S(e,t,o){let l=[{key:"tasks",icon:"\u{1F4CB}",name:"Tasks",interval:300},{key:"improvements",icon:"\u2728",name:"Improvements",interval:600},{key:"coverage",icon:"\u{1F9EA}",name:"Coverage",interval:900},{key:"robustness",icon:"\u{1F6E1}\uFE0F",name:"Robustness",interval:600},{key:"cleanup",icon:"\u{1F9F9}",name:"Cleanup",interval:1200},{key:"commits",icon:"\u{1F4BE}",name:"Commits",interval:900}],n=e.get("autoContinue.enabled",!1),i=e.get("autoApproval.enabled",!0),p=e.get("autoApproval.autoInject",!1),r="";for(let c of l){let d=e.get(`autoContinue.${c.key}.enabled`,!0),u=e.get(`autoContinue.${c.key}.interval`,c.interval),I=e.get(`autoContinue.${c.key}.message`,"");r+=`
			<tr class="${d?"":"disabled"}">
				<td class="cat-icon">${c.icon}</td>
				<td class="cat-name">${c.name}</td>
				<td class="cat-message">
					<input type="text" value="${I}" data-key="autoContinue.${c.key}.message" 
	...
	<div class="section">
		...
		`:`
			<table id="task-table">
				...
				<tbody id="task-tbody">
					${c.map(u=>{let I=u.status==="pending"?"\u23F3":u.status==="in-progress"?"\u{1F504}":"\u2705",h=u.status==="pending"?"Pending":u.status==="in-progress"?"In Progress":"Completed",E=u.status==="pending"?"#cca700":u.status==="in-progress"?"#3794ff":"#89d185";return`
						<tr>
							<td style="cursor: pointer; font-size: 18px;" onclick="cycleStatus('${u.id}', '${u.status}')" title="Click to cycle status">${I}</td>
...
</html>`}async function Ae(e){R=e,b=a.window.createOutputChannel("AI Agent Feedback"),e.subscriptions.push(b),s("INFO","\u{1F680} AI Agent Feedback Bridge activated");let t=m(),o=a.workspace.getConfiguration("aiFeedbackBridge");o.inspect("autoContinue.enabled")?.globalValue!==void 0&&(s("WARN","Detected old Global settings, clearing to use Workspace scope"),await o.update("autoContinue.enabled",void 0,a.ConfigurationTarget.Global));let n=t.get("port");v=await ye(e),s("INFO",`Auto-selected port: ${v} for this window`);let i=a.workspace.name||"No Workspace",p=a.workspace.workspaceFolders?.length||0;s("INFO",`Window context: ${i} (${p} folders)`),$=a.window.createStatusBarItem(a.StatusBarAlignment.Right,100),$.command="ai-feedback-bridge.openSettings",$.show(),e.subscriptions.push($),L=a.window.createStatusBarItem(a.StatusBarAlignment.Right,99),L.command="ai-feedback-bridge.toggleAutoContinue",L.show(),e.subscriptions.push(L),F=a.window.createStatusBarItem(a.StatusBarAlignment.Right,98),F.command="ai-feedback-bridge.injectScript",F.text="$(clippy) Inject",F.tooltip="Copy auto-approval script to clipboard",F.show(),e.subscriptions.push(F),Q(t);let r=a.commands.registerCommand("ai-feedback-bridge.openSettings",async()=>{Ie(e)});e.subscriptions.push(r);let c=a.commands.registerCommand("ai-feedback-bridge.runNow",async()=>{try{let g=await G(e,!0);g?(s("INFO","[Run Now] Manually triggered all enabled reminders"),await O(g,{source:"manual_trigger",timestamp:new Date().toISOString()})):a.window.showInformationMessage("No enabled categories (check settings)")}catch(g){s("ERROR","[Run Now] Failed to send message",{error:g}),a.window.showErrorMessage("Failed to send reminders")}});e.subscriptions.push(c);let d=a.commands.registerCommand("ai-feedback-bridge.injectScript",async()=>{_()});e.subscriptions.push(d);let u=a.commands.registerCommand("ai-feedback-bridge.getPort",()=>v);e.subscriptions.push(u);let I=a.commands.registerCommand("ai-feedback-bridge.addTask",async()=>{let g=await a.window.showInputBox({prompt:"Task title"});if(!g)return;let k=await a.window.showInputBox({prompt:"Task description (optional)"}),w=await a.window.showQuickPick(["bug","feature","improvement","documentation","testing","other"],{placeHolder:"Select category"});await ee(e,g,k||"",w||"other"),await z()});e.subscriptions.push(I);let h=a.commands.registerCommand("ai-feedback-bridge.listTasks",async()=>{let g=await f(e);if(g.length===0){a.window.showInformationMessage("No tasks found");return}let k=g.map(C=>({label:`${C.status==="completed"?"\u2705":C.status==="in-progress"?"\u{1F504}":"\u23F3"} ${C.title}`,description:C.description,task:C})),w=await a.window.showQuickPick(k,{placeHolder:"Select a task to update"});if(w){let C=await a.window.showQuickPick(["Mark as In Progress","Mark as Completed","Mark as Pending","Delete"],{placeHolder:"What do you want to do?"});C==="Delete"?await te(e,w.task.id):C==="Mark as In Progress"?await D(e,w.task.id,"in-progress"):C==="Mark as Completed"?await D(e,w.task.id,"completed"):C==="Mark as Pending"&&await D(e,w.task.id,"pending"),await z()}});e.subscriptions.push(h),Te(e);let E=a.commands.registerCommand("ai-agent-feedback-bridge.sendToCopilotChat",async g=>{g||(g=await a.window.showInputBox({prompt:"Enter feedback to send to Copilot Chat",placeHolder:"Describe the issue or request..."})),g&&await Ee(g,{source:"manual_command",timestamp:new Date().toISOString()})});e.subscriptions.push(E);let V=a.commands.registerCommand("ai-feedback-bridge.toggleAutoContinue",async()=>{let k=m().get("autoContinue.enabled",!1);await H("autoContinue.enabled",!k),s("INFO",`Auto-Continue ${k?"disabled":"enabled"}`),z()});e.subscriptions.push(V);let ie=a.commands.registerCommand("ai-feedback-bridge.changePort",async()=>{let g=await a.window.showInputBox({prompt:"Enter new port number",value:v.toString(),validateInput:k=>{let w=parseInt(k);return isNaN(w)||w<1024||w>65535?"Invalid port (1024-65535)":null}});g&&(await H("port",parseInt(g)),s("INFO",`Port changed to ${g}. Reloading VS Code...`),a.commands.executeCommand("workbench.action.reloadWindow"))});e.subscriptions.push(ie);let re=a.commands.registerCommand("ai-feedback-bridge.showStatus",()=>{let g=m(),k=g.get("autoContinue.interval",300),w=g.get("autoContinue.enabled",!1),pe=`\u{1F309} AI Feedback Bridge Status

Window: ${a.workspace.name||"No Workspace"}
Port: ${v}
Server: ${T?"Running \u2705":"Stopped \u274C"}
Auto-Continue: ${w?`Enabled \u2705 (every ${k}s)`:"Disabled \u274C"}
Endpoint: http://localhost:${v}`;b.appendLine(pe),b.show()});e.subscriptions.push(re);let K=t.get("autoContinue.enabled",!1),P=t.inspect("autoContinue.enabled");s("INFO","[STARTUP] Auto-Continue check:",{enabled:K,defaultValue:P?.defaultValue,globalValue:P?.globalValue,workspaceValue:P?.workspaceValue,workspaceFolderValue:P?.workspaceFolderValue}),K?ne(e):s("INFO","[STARTUP] Auto-Continue is disabled, not starting"),Ne(),e.subscriptions.push(a.workspace.onDidChangeConfiguration(g=>{if(g.affectsConfiguration("aiFeedbackBridge")){let k=m();if(s("DEBUG","Configuration changed",{workspace:a.workspace.name,affectedKeys:["port","autoContinue"].filter(w=>g.affectsConfiguration(`aiFeedbackBridge.${w}`))}),g.affectsConfiguration("aiFeedbackBridge.port")){let w=k.get("port",3737);w!==v&&(s("INFO",`Port change detected: ${v} \u2192 ${w}. Reloading window...`),a.commands.executeCommand("workbench.action.reloadWindow"))}Q(k),g.affectsConfiguration("aiFeedbackBridge.autoContinue")&&Se(e)}})),W=a.chat.createChatParticipant("ai-agent-feedback-bridge.agent",Le),W.iconPath=a.Uri.file(e.asAbsolutePath("icon.png")),e.subscriptions.push(W);let ce=a.commands.registerCommand("ai-agent-feedback-bridge.enableAutoApproval",()=>$e(e));e.subscriptions.push(ce);let de=a.commands.registerCommand("ai-agent-feedback-bridge.disableAutoApproval",()=>Fe());e.subscriptions.push(de);let le=a.commands.registerCommand("ai-agent-feedback-bridge.injectAutoApprovalScript",()=>Re());e.subscriptions.push(le),s("INFO",`Feedback server started on http://localhost:${v}`)}function Q(e){if(!L||!$)return;let t=e.get("autoContinue.enabled",!1);$.text=`AI Dev: ${v}`,$.tooltip="Click to configure AI Feedback Bridge",t?(L.text="$(sync~spin) Stop AI Dev",L.tooltip=`Auto-Continue active
Click to stop`):(L.text="$(play) Start AI Dev",L.tooltip=`Auto-Continue inactive
Click to start`)}async function G(e,t=!1){let o=m(),l=["tasks","improvements","coverage","robustness","cleanup","commits"],n=Date.now(),i=[],p="autoContinue.lastSent",r=e.globalState.get(p,{}),c={...r};for(let d of l){let u=o.get(`autoContinue.${d}.enabled`,!0),I=o.get(`autoContinue.${d}.interval`,300),h=o.get(`autoContinue.${d}.message`,"");if(!u||!h)continue;let E=r[d]||0,V=(n-E)/1e3;(t||V>=I)&&(i.push(h),c[d]=n)}return await e.globalState.update(p,c),i.length===0?"":i.join(". ")+"."}function ne(e){if(m().get("autoContinue.enabled",!1)){let n=a.workspace.name||"No Workspace";s("INFO",`\u2705 Auto-Continue enabled for window: ${n}`),A=setInterval(async()=>{try{if(!m().get("autoContinue.enabled",!1)){s("INFO","[Auto-Continue] Detected disabled state, stopping timer"),A&&(clearInterval(A),A=void 0);return}let r=await G(e);r&&(s("INFO","[Auto-Continue] Sending periodic reminder"),await O(r,{source:"auto_continue",timestamp:new Date().toISOString()}))}catch(i){s("ERROR","[Auto-Continue] Failed to send message",{error:y(i)})}},500)}else s("DEBUG","Auto-Continue is disabled")}function Se(e){A&&(clearInterval(A),A=void 0,s("INFO","Auto-Continue timer stopped")),ne(e)}async function Le(e,t,o,l){b.appendLine(`Chat request received: ${e.prompt}`),o.markdown(`### \u{1F504} Processing Feedback
...
`))}return{metadata:{command:"process-feedback"}}}async function O(e,t){try{let o=`# \uFFFD AI DEV MODE
...
</html>`}async function Pe(){T&&(T.close(),s("INFO","HTTP server closed")),A&&(clearInterval(A),A=void 0,s("INFO","Auto-continue timer cleared")),x&&(clearInterval(x),x=void 0,s("INFO","Auto-approval interval cleared")),R&&await xe(R,v),s("INFO","\u{1F44B} AI Agent Feedback Bridge deactivated")}0&&(module.exports={activate,deactivate});
